  ...
  <div class="login-wrap">
    <div class="login">
      <div class="loginform">
        ...
        <div class="content">
          <form action="##" @submit="getLogin($event)">
            <div class="input-text clearFix">
              <i></i>
              <input type="text" placeholder="邮箱" v-model="email" />
              <span class="error-msg" v-show="isShowEmail">错误提示信息</span>
            </div>
...
export default {
  ...
  methods: {
    async getLogin(e) {
      e.preventDefault()
      try {
        const result = await this.$store.dispatch('register_login/getLogin', { email: this.email, password: this.password })
        if (result.code === 200) {
          // 登录成功就跳转到home
...
.login-wrap .loginform .content form .btn_login {
